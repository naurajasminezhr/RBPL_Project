...
<body style="background-color: #F2F2F2">
    ...
    <form id="profile-form" method="POST" action="{{ route('save-profile') }}" enctype="multipart/form-data" class="form">
        @csrf
        ...
        <div class="file-input-wrapper">
            <input type="file" id="profile_picture" name="profile_picture" id="profilePicture" accept="image/*" required
                class="file-input">
            <button type="submit" class="custom-file-button" style="font-family:Lexend">Upload Photo</button>
        </div>
...
<!--
    <script>
        ...
        document.getElementById('profile-form').addEventListener('submit', function(event) {
            ...
            // Submit the form to 'save-profile' route
            fetch('{{ route('save-profile') }}', {
                    method: 'POST',
                    body: new FormData(event.target)
                })
                .then(response => response.json())
                .then(data => {
                    // Handle response from 'save-profile' route
                    console.log(data);
                })
                .catch(error => {
                    // Handle error
    ...
    -->
        ...
        document.querySelector('.custom-file-button').addEventListener('click', function() {
            ...
            if (input.files && input.files.length > 0) {
                ...
                // Submit the form to 'profilepicture.upload' route
                fetch('{{ route('profilepicture.upload') }}', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => response.json())
                    .then(data => {
                        // Handle response from 'profilepicture.upload' route
                        console.log(data);
                    })
                    .catch(error => {
                        // Handle error
